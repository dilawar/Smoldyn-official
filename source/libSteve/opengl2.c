...
void SpecialKeyPush2(GLFWwindow* win, unsigned char key, int x, int y, int mods);

void SpecialKeyPush(GLFWwindow* win, int key, int x, int y, int mods);
...
/* KeyPush */
static void KeyPush(GLFWwindow* window, int key, int scanmode, int action, int mods)
{
#ifdef __gl_h_
    ...
    if(key == 'Q') {
        ...
        w = viewport[2];
        h = viewport[3];
        snprintf(str, 2 * STRCHAR, "%s%%0%ii.tif", TiffName,
...
/* SpecialKeyPush2 */
void SpecialKeyPush2(GLFWwindow* win, unsigned char key, int x, int y, int mod)
{
#ifdef __gl_h_
    GLfloat m[16];
    GLint   viewport[4], w, h;
    ...
    char* wname, float xlo, float xhi, float ylo, float yhi, float zlo, float zhi)
{
    ...
    GLFWwindow* window = glfwCreateWindow(w, h, "Smoldyn", NULL, NULL);
    if(!window) {
    ...
    glfwSetKeyCallback(window, KeyPush);
...
void gl2SetKeyPush(GLFWwindow* win, unsigned char key)
{
    if(strchr(" QT0=-xyzXYZ", key))
        KeyPush(win, key, 0, 0, 0);
    else if(strchr("durlDURL", key))
        SpecialKeyPush2(win, key, 0, 0, 0);
    return;
}
